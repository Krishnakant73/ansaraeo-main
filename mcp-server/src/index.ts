...
function getSupabase(): SupabaseClient {
  ...
  if (!url || !key) {
    throw new Error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY in the MCP server environment."
    );
  }
...
server.tool(
  "aeo_list_brands",
  "List every brand in the AnsarAEO workspace (id, name, domain, org_id).",
  {},
  async () => {
...
server.tool(
  "aeo_visibility_summary",
  "Compute the recent brand-mention rate across all visibility runs for a brand.",
...
server.tool(
  "aeo_list_competitors",
  "List confirmed competitors for a brand.",
  { brandId: z.string().describe("Brand UUID") },
  async ({ brandId }) => {
    const supabase = getSupabase();
    const { data, error } = await supabase
