...
import { INTENTS, type IntentKey } from "@/lib/intent";
...
// ============================================================
// Brand workspace › Visibility tab body.
//
// Same content as the old /dashboard/b/[slug]/visibility page. The
// caller (either the workspace tab render or the thin-wrapper page)
// resolves the brand + slug; we own the visibility computation and
// panel rendering.
// ============================================================

export default async function VisibilityBody({ brand, slug }: { brand: Brand; slug: string }) {
  ...
  const priorityPromptIds = new Set((prompts ?? []).filter((p) => p.priority).map((p) => p.id));
  ...
  try {
    ...
    winReasons = reasons
      .slice(0, 8)
      .map((r) => ({ competitor: r.competitor, promptText: r.promptText, reason: r.reason }));
  } catch {
    winReasons = [];
  }
  ...
  return (
    <div>
      ...
      <PageHeader
        title={brand.name}
        subtitle={`${brand.industry ?? "AI search visibility"} · Brand › Visibility`}
        actions={
          <>
            <Link href={`/dashboard/b/${slug}/competitors`} className="btn-secondary !h-10 !px-5">
              Competitors
            </Link>
            <Link href={`/dashboard/b/${slug}/prompts`} className="btn-primary !h-10 !px-5">
              Manage prompts
            </Link>
          </>
        }
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2" title="Visibility trend" description="Daily mention rate across tracked prompts">
        ...
        >
          {winReasons.length === 0 ? (
            <p className="text-sm text-muted">
              No competitor wins recorded yet — add confirmed competitors to start tracking.
            </p>
          ) : (
              ...
              {winReasons.map((w, i) => (
                <li key={i} className="flex items-start justify-between gap-3 text-sm">
                  <span className="min-w-0">
                    ...
                    <span className="text-muted"> — &ldquo;{w.promptText}&rdquo;</span>
                  </span>
        ...
        <Panel title="AI-driven revenue" description="Outcome layer — only shown when GA4 / Shopify are connected">
          {outcome?.connected ? (
            <div className="grid gap-4 sm:grid-cols-3">
              <StatCard
                label="AI-referral sessions (30d)"
                value={outcome.aiSessions ?? "—"}
                icon={<Eye className="h-4 w-4" />}
              />
              <StatCard
                label="Assisted revenue (30d)"
                value={
                  outcome.assistedRevenue === null
                    ? "—"
                    : `₹${Math.round(outcome.assistedRevenue).toLocaleString("en-IN")}`
                }
                icon={<TrendingUp className="h-4 w-4" />}
              />
              <StatCard
                label="Attributed orders (30d)"
                value={outcome.orders ?? "—"}
                icon={<Activity className="h-4 w-4" />}
              />
            </div>
          ) : (
