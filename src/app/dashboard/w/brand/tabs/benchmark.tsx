...
export default async function BenchmarkBody({ brand }: { brand: Brand }) {
  ...
  const [industryVis, overallVis, industryMention] = await Promise.all([
    cachedBenchmark(`cell|industry|${industryCategory}|avg_visibility|${period}`, () =>
      getBenchmarkCell({
        ...
        dimensionValue: industryCategory,
        metric: "avg_visibility",
        periodStart: period,
      }),
    ),
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`How ${brand.name}'s AI visibility compares to the ${industryLabel(
          industryCategory,
        )} industry — anonymously aggregated across all AnsarAEO brands.`}
      />
