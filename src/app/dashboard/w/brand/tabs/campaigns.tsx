...
import CampaignPanel from "@/components/dashboard/objects/CampaignPanel";
...
export default async function CampaignsBody({ brand }: { brand: Brand }) {
  ...
  if (!brand) {
    return (
      <EmptyState
        ...
        description="Select a brand to manage campaigns."
      />
    );
  }
      ...
      <Panel title={`Campaigns (${campaigns?.length ?? 0})`}>
        {!campaigns?.length ? (
          ...
          <ul className="divide-y divide-line">
            {campaigns.map((c: { id: string; name: string; objective: string | null; status: string }) => (
              <li key={c.id} className="flex items-center justify-between gap-4 py-3">
                <div className="flex items-center gap-2">
                  <Sheet>
                    <SheetTrigger asChild>
                      <button className="text-left outline-none" title="Peek campaign">
                        <span className="block text-sm font-medium text-ink transition-colors hover:text-accent hover:underline">
                          {c.name}
                        </span>
                        {c.objective && <span className="block text-xs text-muted">{c.objective}</span>}
                      </button>
                    </SheetTrigger>
                    <SheetContent>
                  ...
                  <a
                    href={`/dashboard/w/campaign/${c.id}/overview`}
