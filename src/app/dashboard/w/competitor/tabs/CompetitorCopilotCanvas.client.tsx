...
// ============================================================
// CompetitorCopilotCanvas — dedicated Copilot canvas for a competitor.
// Mirrors PromptCopilotCanvas: SSE stream, workspace context payload,
// suggested prompts scoped to competitive intel.
// ============================================================

type Message = { role: "user" | "assistant"; content: string };

const SUGGESTED = [
  "Where is this competitor winning against us?",
  "Suggest 3 outreach targets to close the gap",
  "Draft a battlecard section vs this competitor",
  "Which of our prompts are they contesting most?",
];

export default function CompetitorCopilotCanvas({
  competitorId,
  competitorName,
  brandName,
}: {
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              ...
              summary: `Competitor "${competitorName}" tracked against brand "${brandName}".`,
              hints: [
                "Answer from real visibility_runs + citations data only.",
                "Never invent share-of-voice numbers, positions, or citations.",
                "For content drafts, ship [ADD ...] placeholders — no fabricated specifics.",
              ],
            },
          },
        }),
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about {competitorName}:</p>
