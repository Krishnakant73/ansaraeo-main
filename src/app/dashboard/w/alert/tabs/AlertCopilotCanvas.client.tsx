...
const SUGGESTED = [
  "Is this rule too sensitive?",
  "What caused the last firing?",
  "Should I widen the window or the threshold?",
  "Summarize recent firings",
];

export default function AlertCopilotCanvas({
  alertId,
  metric,
  brandName,
}: {
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              ...
              id: alertId,
              label: metric,
              summary: `Alert rule "${metric}" for brand "${brandName}".`,
              hints: [
                "Answer from geo_alert_rules + geo_alert_firings only.",
                "Never invent thresholds, firing counts, or metric values.",
              ],
            },
          },
        }),
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about this alert rule:</p>
