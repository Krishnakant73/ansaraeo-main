...
const SUGGESTED = [
  ...
  "What's missing to close the verify loop?",
  "Is the trigger too aggressive?",
];

export default function PlaybookCopilotCanvas({
  playbookId,
  playbookName,
  orgName,
  trigger,
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              ...
              id: playbookId,
              label: playbookName,
              summary: `Playbook "${playbookName}" in org "${orgName}". Trigger: ${trigger}. ${stepCount} step${stepCount === 1 ? "" : "s"}.`,
              hints: [
                "Answer from the playbook row (steps + trigger + active state) only.",
                "Playbooks are templates — do not claim they have already run.",
                "Suggest specific improvements to the step sequence when asked.",
              ],
            },
          },
        }),
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about this playbook:</p>
