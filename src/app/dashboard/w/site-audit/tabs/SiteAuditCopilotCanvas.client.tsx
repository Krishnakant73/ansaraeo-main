...
const SUGGESTED = [
  "What's my biggest fix?",
  "Is llms.txt actually helping AI engines find us?",
  "Why did the score drop vs last audit?",
  "Which check gives the best ROI to fix first?",
];

export default function SiteAuditCopilotCanvas({
  auditId,
  brandName,
  overallScore,
  failCount,
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              kind: "site-audit",
              id: auditId,
              label: `Site audit for ${brandName}`,
              summary: `Site audit snapshot for ${brandName}. Overall score ${overallScore ?? "—"} / 100. ${failCount} failing check(s), ${warnCount} warning check(s).`,
              hints: [
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about this audit:</p>
