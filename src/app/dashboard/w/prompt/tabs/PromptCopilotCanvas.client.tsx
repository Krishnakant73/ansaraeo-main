...
// ============================================================
// PromptCopilotCanvas — a larger conversation surface than the docked
// Copilot, scoped to the current prompt. It POSTs to /api/agent/chat
// with Accept: text/event-stream (same contract as CopilotDock) and
// includes `context.workspace = { kind: "prompt", id, label, summary }`
// so the server route can ground on the exact prompt without parsing
// the URL. Zero import from UWE core — the tab body passes plain data.
// ============================================================
...
const SUGGESTED = [
  "Why isn't this prompt getting cited?",
  "Draft an answer block for this prompt",
  "Which competitor is winning this prompt and why?",
  "Suggest 3 similar prompts I should track",
];

export default function PromptCopilotCanvas({
  promptId,
  promptText,
  brandName,
}: {
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              kind: "prompt",
              id: promptId,
              label: promptText.slice(0, 100),
              summary: `Prompt "${promptText}" tracked for brand ${brandName}.`,
              hints: [
                "Answer from this prompt's visibility_runs and citations only.",
      ...
      while (true) {
        ...
        for (const evt of events) {
          ...
          try {
            const parsed = JSON.parse(raw) as { type: string; delta?: string; error?: string };
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about this prompt — try one of these:</p>
