...
const SUGGESTED = [
  "Which prompts does this engine miss most?",
  "How does its citation posture compare to Perplexity?",
  "Why did the mention rate change this week?",
  "Suggest 3 content drafts tuned to this engine",
];

export default function EngineCopilotCanvas({
  engineName,
  engineDisplay,
  brandName,
}: {
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              ...
              id: engineName,
              label: engineDisplay,
              summary: `Engine "${engineDisplay}" answering about brand "${brandName}".`,
              hints: [
                "Answer from real visibility_runs + citations for this engine only.",
                "Never invent engine behavior; if a signal is missing, say so.",
    ...
    <div className="flex min-h-[520px] flex-col rounded-2xl border border-line bg-white">
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about {engineDisplay}:</p>
      ...
      >
        <input
          ...
          disabled={streaming}
          placeholder={`Ask about ${engineDisplay}…`}
