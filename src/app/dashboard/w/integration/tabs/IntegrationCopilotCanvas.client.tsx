...
const SUGGESTED = [
  ...
  "What data does this provider give us?",
  "How do I reconnect if the token expired?",
];

export default function IntegrationCopilotCanvas({
  integrationId,
  providerName,
  status,
  brandName,
        ...
        body: JSON.stringify({
          ...
          context: {
            workspace: {
              kind: "integration",
              id: integrationId,
              label: `${providerName} for ${brandName}`,
              summary: `${providerName} integration on brand ${brandName}. Status: ${status}. Credentials ${isEncrypted ? "encrypted" : "plaintext (legacy row)"}.`,
              hints: [
                "Answer from the integration row only.",
                "Never claim to see decrypted credential values — this workspace does NOT decrypt.",
                "Never invent scope, tokens, or provider capabilities.",
              ],
            },
          },
        }),
    ...
    <div className="flex min-h-[520px] flex-col rounded-2xl border border-line bg-white">
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-muted">Ask anything about this integration:</p>
      ...
      >
        <input
          ...
          disabled={streaming}
          placeholder="Ask about this integration…"
